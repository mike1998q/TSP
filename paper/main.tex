...
(Fig.~\ref{fig:arch}) whose design follows a single discipline --- trace
each recurring weakness of current forecasters to one matching structural
remedy. Three deficiency--remedy pairs organize the model:
\emph{(i)~deep networks relearn, and on small data never recover, the
linear maps that already dominate ETT-class benchmarks} $\Rightarrow$
\emph{each branch carries an explicit linear anchor with zero-initialized
nonlinear corrections}; \emph{(ii)~single-view models --- time-only SSMs or
spectral-only mixers --- underuse the complementary domain} $\Rightarrow$
\emph{parallel time and frequency branches, fused by a convex gate that
preserves both prediction paths}; \emph{(iii)~uniform capacity
over-parameterizes low-channel data yet under-serves high-channel data}
$\Rightarrow$ \emph{a bidirectional variate-mixing Mamba switched on per
dataset}. We elaborate each pair below.
...
ablations find the frequency branch's contribution significant on one of
three ablated datasets (Solar) and within noise on the other two
(Sect.~\ref{sec:ablation}); we accordingly claim per-dataset
...
\cite{rlinear,revin} (Sect.~\ref{sec:anchors}). We adopt this for the
interpretable linear start; a controlled comparison against standard
random initialization finds it accuracy-neutral on the two datasets
tested (Sect.~\ref{sec:ablation}), so we position it as an
interpretability default, not an accuracy mechanism.
...
Crucially, capacity is \emph{placed} per dataset, and the paired
ablations support this in both directions: removing the variate mixer
hurts weather, while \emph{adding} it hurts ETTh1 --- each with a
confidence interval excluding zero (Sect.~\ref{sec:ablation}).
...
these methods model \emph{level} but not \emph{scale}: when the variance of
a series changes over time --- heteroscedasticity --- the fluctuation is
pushed into the irregular term and left unmodeled, which is exactly the
regime of the non-stationary forecasting benchmarks. \model{} inherits this
lineage but assigns each classical component a dedicated, learnable home.
Its time branch performs moving-average trend/seasonal decomposition in the
spirit of DLinear \cite{dlinear}, so the seasonal-trend split lands on the
\emph{linear anchors} (Sect.~\ref{sec:anchors}); its RevIN instance
normalization \cite{revin} removes each window's mean and standard
deviation, absorbing precisely the dispersion that the additive triad
leaves behind, so the anchors operate on an approximately stationary,
homoscedastic residual. Read this way, normalization is not a
preprocessing detail but the component that handles heteroscedasticity ---
which is why our ablations find it decisive yet dataset-dependent
...
structure. The same discipline of routing a specific deficiency to a
dedicated spectral or normalization operator, rather than applying one
mechanism uniformly, has been argued for frequency-domain design in
adjacent fields \cite{dfir}.
...
de-normalization. Since $\mathbf{Y}$ lies elementwise
...
(Table~\ref{tab:ablation}, Fig.~\ref{fig:evidence}b) quantify the two
domains directly on solar:
...
contributions. Figure~\ref{fig:evidence}a reads out these weights on a
trained ETTh1 model: initialized time-heavy at $g_0{\approx}0.9$, the gate
settles below the equal-mix line for most variates, shifting weight toward
the frequency branch --- the same branch the ETTh1 ablation certifies as
sufficient. The gate readout and the ablation thus agree, though only the
ablation is causal; a systematic study of gate behavior across all
...
what the paired ablations do and do not support: capacity placement is
validated in \emph{both} directions (the variate mixer helps weather yet
hurts ETTh1; normalization helps weather yet hurt solar), the temporal
encoder helps weather, and the frequency branch contributes measurably
on Solar while being redundant on ETTh1 and weather; zero initialization
is accuracy-neutral in a controlled comparison. We therefore claim
per-dataset structural flexibility with dataset-dependent branch value,
not a universal benefit of combining domains. They also surface a transferable methodological caution:
...
it. (1)~The frequency-branch gain is demonstrated on a single dataset
(Solar) at one horizon with $n{=}3$; extending the paired matrix across
horizons and the remaining datasets would establish where it
generalizes. (2)~Zero initialization is accuracy-neutral in our
controlled comparison, but its hypothesized stability value remains
untested --- convergence curves, failure rates, and the magnitude of the
learned corrections are open analyses. (3)~Baseline numbers are quoted
from \cite{smamba} rather than re-run; the in-framework DLinear re-run
(Sect.~\ref{sec:repro}) exposes a $+0.006$ implementation gap, so
comparative claims should be read as ``competitive with reported results
under the same protocol'' until S-Mamba, iTransformer, and PatchTST are
reproduced in this framework. Architecturally: the fixed look-back of 96 hides cycles

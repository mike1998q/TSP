...
comparisons. Paired ablations at $H{=}96$ show that the frequency path is
significant on Solar but redundant on ETTh1 and Weather; the variate mixer
helps Weather yet hurts ETTh1; RevIN helps Weather yet hurts Solar; and zero
initialization is accuracy-neutral. The evidence supports conditional
component placement rather than a universal dual-domain gain.
...
Paired ablations find the frequency path significant on Solar and within
noise on ETTh1 and Weather, so the dual-domain claim is explicitly
conditional. A causal Mamba models time when useful, and a bidirectional
Mamba mixes variates only in selected configurations; removing that mixer
hurts Weather, whereas adding it hurts ETTh1.

\noindent\textbf{Contributions.} \textbf{(1)}~A dual-domain architecture
whose output is a gated convex combination of independently testable branch
forecasts; \textbf{(2)}~a DLinear-style time backbone with zero-initialized
deep corrections and an optional dormant spectral pathway, with a controlled
initialization ablation; \textbf{(3)}~empirical evidence that state-space and
normalization choices can help or hurt across the tested datasets; and
\textbf{(4)}~three-seed results on nine benchmarks plus paired
ablations on three datasets, including statistically supported negative and
...
\paragraph{Dual-domain evidence.} The clearest result is on Solar: removing
the frequency branch degrades MSE by $+0.0114$ with a CI excluding zero ---
the only frequency-branch comparison whose paired CI excludes zero in our
...
$-0.0007$). The dual-domain claim is thus conditional: the frequency path
contributes measurably on Solar, while the current tests detect no
branch-removal effect on ETTh1 or Weather.

\paragraph{Capacity choices can help or hurt.} Adding the variate
...
contribution is statistically supported. ETTh1 and Weather instead show
substantial branch redundancy. Because Solar's 10-minute daily cycle spans
$144{>}L{=}96$ samples, the spectral path cannot observe a complete daily
period; its measured benefit should therefore be attributed to shorter-scale
spectral structure, not to recovery of the full daily cycle.
...
datasets, and trails on Exchange and Traffic. Paired ablations support capacity
placement in both directions: the variate mixer helps Weather yet hurts
ETTh1, normalization helps Weather yet hurts Solar, and the frequency branch
has a confidence interval excluding zero on Solar in the exploratory paired
analysis. Zero initialization is
accuracy-neutral in the current tests and is retained only for its controlled
linear starting point. These findings support conditional component
placement, not universal superiority of every module.
...
only $H{=}96$ and three seeds; schedule sensitivity and head-norm dynamics are
not measured; and the complexity table lacks wall-clock and matched-baseline
runtime. The fixed 96-step look-back also leaves the Traffic weekly-cycle
hypothesis untested, while Exchange at $H{=}720$ remains a notable failure
case. Addressing these points requires unified baseline reruns, a second
ablation horizon, schedule controls, runtime profiling, and longer-look-back
experiments. Explicit forecasting of time-varying dispersion is left as
future work rather than claimed as part of the evaluated model.

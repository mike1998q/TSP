...
reported S-Mamba results on five (up to 3.5\% on ETTh1), tying three within run-to-run noise, and trailing one, with far
fewer parameters on small datasets. Ablations show clear
...
the released code makes both requirements one command each: a multi-seed
main-results runner (horizons $\times$ seeds with mean~$\pm$~std
aggregation) and an in-framework DLinear baseline trained through the
identical data pipeline, splits, schedule, and evaluation script, as a
first step toward the unified baseline re-run.

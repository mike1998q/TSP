...
gate. Two guidelines organize the design. First, a DLinear-style
time backbone is trained alongside zero-initialized nonlinear
corrections; the optional FITS-style spectral map starts dormant, so the
initial prediction is linear in the instance-normalized window. Second,
state-space capacity is placed per dataset: a bidirectional Mamba over
variates is enabled where cross-channel structure helps and disabled where
it overfits. Across nine look-back-96 benchmarks and three seeds,
\model{} beats reported baselines by more than the $0.005$ descriptive
tolerance on Solar, ETTh1, and ETTh2 --- only Solar and ETTh1 also clear the
wider $0.01$ cross-paper caution --- lies within tolerance of the leading
reported results on four further datasets, and trails on Traffic and
Exchange. Because baseline
numbers are quoted rather than uniformly re-run, these are cross-paper
comparisons. Paired ablations show the variate mixer helps
...
27-cell branch-ablation matrix (nine datasets; four horizons on six) finds
the frequency path significant on four datasets at the 96-step horizon,
fading at longer ones, the time branch necessary on ETTm1 at every horizon,
and the branches redundant elsewhere; zero initialization is
accuracy-neutral. The evidence establishes conditional component placement:
dual-domain value is conditional on dataset and horizon.

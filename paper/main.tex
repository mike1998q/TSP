...
comparisons. The primary ablation finding is two-sided and cross-dataset:
paired tests at $H{=}96$ show the variate mixer helps Weather yet hurts
ETTh1, and RevIN helps Weather yet hurts Solar --- capacity and
normalization must be \emph{placed}, not defaulted. The frequency path is
significant only on Solar (one marginal cell, labeled exploratory) and
redundant elsewhere; zero initialization is accuracy-neutral. The evidence
establishes conditional component placement; the dual-domain benefit remains
dataset-specific pending a full branch-ablation matrix.
...
pattern. The frequency path is supported on a single dataset only (Solar,
with a CI barely excluding zero) and is within noise on ETTh1 and Weather,
so the dual-domain claim is explicitly conditional and exploratory.
...
forecasts, with its single-dataset support reported as exploratory; and
\textbf{(4)}~three-seed results on nine benchmarks plus paired ablations on
three datasets, including statistically supported negative and
dataset-dependent findings, with a released harness for the full
branch-ablation matrix.
...
\paragraph{Branch-level evidence is exploratory.} By contrast, the
frequency-branch case rests on one cell: on Solar, removing the
frequency branch degrades MSE by $+0.0114$ with a CI of $[+0.001,+0.021]$ ---
the only frequency-branch comparison whose paired CI excludes zero, and its
lower bound barely does. On ETTh1 and Weather both single-branch variants lie
within their CIs (on ETTh1 the frequency branch \emph{alone} even matches the
full model, $-0.0007$). A single marginal result on one dataset at one
horizon cannot carry a dual-domain conclusion, so we report it as
exploratory: the frequency path is an optional component with measured
support on Solar and no detected branch-removal effect elsewhere. The
released harness includes a branch-ablation matrix (full / time-only /
freq-only on all nine datasets and four horizons, paired seeds) as the
designated resolving experiment.
...
\paragraph{Where each domain wins.} On Solar, removing the frequency branch
increases MSE by $+0.0114$ with a CI excluding zero, whereas removing the time
branch yields $+0.0051$ with a CI that includes zero
(Table~\ref{tab:ablation}). Thus, at $n{=}3$, only the frequency-path
contribution is statistically supported --- and only in this one cell, which
is why Sect.~\ref{sec:ablation} labels it exploratory rather than load-bearing.
ETTh1 and Weather instead show substantial branch redundancy. Because Solar's
10-minute daily cycle spans $144{>}L{=}96$ samples, the spectral path cannot
observe a complete daily period; its measured benefit should therefore be
attributed to shorter-scale spectral structure, not to recovery of the full
daily cycle.
...
datasets, and trails on Exchange and Traffic. The conclusion the ablations
robustly support is \emph{placement}: the variate mixer helps Weather yet
hurts ETTh1 and normalization helps Weather yet hurts Solar, each direction
backed by a paired CI excluding zero on a different dataset. The frequency
branch, in contrast, is supported only by a single marginal Solar cell and is
reported as exploratory; zero initialization is accuracy-neutral and retained
only for its controlled linear starting point. These findings establish
conditional component placement, not universal superiority of every module
and not a general dual-domain gain.

The evidence remains limited in several ways. Baselines other than DLinear on
ETTh1 are quoted rather than rerun in one pipeline; component ablations cover
only $H{=}96$ and three seeds; the frequency-branch question in particular
needs the full branch-ablation matrix (full / time-only / freq-only across
all nine datasets and four horizons, paired seeds), for which the released
harness provides a resumable runner; schedule sensitivity and head-norm
dynamics are not measured; and the complexity table lacks wall-clock and
matched-baseline runtime. The fixed 96-step look-back also leaves the Traffic
weekly-cycle hypothesis untested, while Exchange at $H{=}720$ remains a
notable failure case. Explicit forecasting of time-varying dispersion is left
as future work rather than claimed as part of the evaluated model.
